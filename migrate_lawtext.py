...
# Step 1: Create the lawtext table in PostgreSQL
create_table_sql = '''
...
mysql_cursor.execute("SELECT * FROM lawtext")
...
INSERT INTO lawtext (
    ...
    chronologielink, changeslink, sourcelink, quelllink
) VALUES %s
'''
...
with postgres_conn.cursor() as cursor:
    cursor.execute("SELECT MAX(id) FROM lawtext")
    max_id = cursor.fetchone()[0]
    if max_id is not None:
        cursor.execute(
            "SELECT setval(pg_get_serial_sequence('lawtext', 'id'), %s, true)",
            (max_id,)
        )
