...
# Step 1: Create the errorlog table in PostgreSQL
create_table_sql = '''
...
mysql_cursor.execute("SELECT * FROM errorlog")
...
with postgres_conn.cursor() as cursor:
    cursor.execute("SELECT MAX(id) FROM errorlog")
    max_id = cursor.fetchone()[0]
    if max_id is not None:
        cursor.execute(
            "SELECT setval(pg_get_serial_sequence('errorlog', 'id'), %s, true)",
            (max_id,)
        )
